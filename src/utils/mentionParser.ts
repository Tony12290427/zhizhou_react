 ...
 * @param nickname - 用户昵称
 */
function escapeMentionNickname(nickname: string): string {
  if (!nickname) return ''
  return nickname
}
...
export function parseMentions(text: string | null | undefined): string {
  ...
  let protectedText = text.replace(mentionLinkRegex, (match: string) => {
  ...
  const atMarkers: string[] = []
  protectedText = protectedText.replace(atMarkerRegex, (match: string) => {
  ...
  let result = escapedText.replace(mentionRegex, (_match: string, nickname: string, userId: string) => {
    // 转义昵称和用户ID，防止XSS攻击
    const escapedNickname = escapeMentionNickname(nickname)
    const escapedUserId = escapeHtml(userId)
    // 生成用户主页链接，使用知舟号作为路由参数
 ...
 * @returns 用户ID数组
 */
export function extractMentionedUsers(text: string | null | undefined): MentionedUser[] {
  if (!text) return []
 ...
 * @returns 清理后的文本
 */
export function cleanMentions(text: string | null | undefined): string {
  if (!text) return ''
